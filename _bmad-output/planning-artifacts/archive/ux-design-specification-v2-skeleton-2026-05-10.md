...
> **REVISED 2026-05-10** via Sprint Change Proposal (`sprint-change-proposal-2026-05-10.md`).
> Risograph + Neobrutalist 시스템은 **deprecated**. 정본 Risograph 사양은
> `archive/ux-design-specification-v1-risograph-2026-05-10.md` 보존.
> 본 섹션은 *결정된 골격* 만 명시; 구체 palette/typography/motion/component 값은
> `/bmad-create-ux-design` 신규 라운드 산출물로 land.

### Design System Choice

**yeolsal v2 — Oxblood Editorial** (Custom Design System, Dark Luxury × Editorial 융합).
키 컬러 oxblood (red-adjacent warm), 다크 럭셔리 베이스, editorial typography hierarchy.
Established RN 라이브러리 부적합 (Material/iOS 디폴트와 충돌, 차별화 죽음).
Token-driven 4-layer 구조 + FE→BE codegen sync (Architecture §4.16).

### Rationale for Selection

- **PRD §2.3 strategic bet #5** — brand uniqueness as 5축 차별화 #5는 *유지*; 표현만 swap.
- **Dignity tone 보존** — oxblood는 red-adjacent warm으로 alarm/blood 미학 회피
  (PRD §6.1 dignity-color guard 준수). RED를 *실패 신호*로 쓰지 않음.
- **Final-3 포스터 share-rate** — 새 직진의 falsification trigger
  (Day-30 share-rate < 15% surviving members → 시각 방향 재검토).
- **8주 budget fit** — W1-W3 design lock + base atom build, W4-W7 feature build.
  Phase-1.5 컨틴전시 폴리시 일부 미루는 옵션 인지.

### Implementation Approach — 4-Layer Token-Driven (구조 유지)
...
│   WelcomeWindow · RevivalSequence · ReceivedGiftToast      │
├────────────────────────────────────────────────────────────┤
│ L3 Component Layer (v2 atoms / molecules)                  │
│   이름·prop·anatomy는 design 라운드 산출물로 lock           │
│   기존: Button / Card / Sheet / GrassGrid / SystemMessage  │
│   신규: KudosButton · RitualMoment · PoolMeter             │
│   재평가: NoiseOverlay (Risograph 전용 — keep/cut TBD)     │
├────────────────────────────────────────────────────────────┤
│ L2 Primitive Layer (RN + SVG)                              │
│   React Native core · react-native-svg ·                   │
│   subtle blur / metallic accent (dark luxury 허용)         │
├────────────────────────────────────────────────────────────┤
│ L1 Token Layer (FE→BE codegen via tokens.json — §4.16)     │
│   color · semantic.survival (packed type) ·                │
│   semantic.emotion · typography.persona · motion.narrative │
│   spacing · border · shadow · subMode override             │
└────────────────────────────────────────────────────────────┘
```

**L1 신규 토큰 (구조 — 값은 design 라운드 lock)**:
- `semantic.survival` — **packed type** `{ color, label, icon, grass-treatment }`
  per state (`ACTIVE` / `YELLOW` / `RED` / `SPECTATOR`). NFR-9.6.1 type-level
  enforcement: 소비 코드는 color 필드를 label 없이 참조 불가; CI lint hard gate.
- `semantic.emotion` — 7 design principle 매핑 (구체 값 TBD via design 라운드).
- `typography.persona` — 1인칭 복수 톤 가이드 + Editorial wordmark 규약.
- `motion.narrative` — 서사 motion 토큰 (M3 부활 시퀀스 / ritual moment / footnote fade).
- `subMode` override 집합 — `editorial` / `bento` / `quiet` / `postcard` / `plate`
  (5 candidates; design 라운드 mockup 후 surface 매핑 lock).

**L2 RN 제약 (유지)**:
- ❌ `react-native-skia` 추가 cut — native rebuild + 8주 budget 압박.
- ❌ Reanimated 3 layer 합성 cut — *(a) 5단계 정적 SVG/PNG swap* 또는 design 라운드 대체안.
- ✅ `react-native-svg` 이미 deps — vector primitive 1차 매체.
- ⚠ Hard-offset shadow only 가드 **해제** — dark luxury 톤은 subtle blur / metallic
  accent 허용; design 라운드에서 shadow scale 재정의.

### v1 Component Build Matrix

이름·anatomy는 §Component Strategy로 이동 (disposition + story ownership lock 포함).
구체 spec은 design 라운드 후 재작성.

### Customization Strategy

- **Brand voice as system constraint** (PRD FR-8.8.6) — 유지.
- **Accessibility (NFR-9.6.*)** — color 단독 carrier ❌; `semantic.survival` packed type
  + CI lint hard gate (Architecture §4.15)로 enforce. WCAG 2.2 AA contrast는 새 palette
  기준 재검증 필수. Dynamic Type 1.0–1.5x. Reduced motion fallback (Story 1.7 RitualMoment AC).
- **Native module 정책** — W5 KakaoTalk SDK가 v1 마지막 native add. 그 외 cut.
- **Theming** — v1 단일 다크 (oxblood 키). Light mode는 v2 이후. Themed-room preset
  (운동방/공부방/글쓰기방)은 v1.5 이후 (deferred).
...
> **REVISED 2026-05-10**. v1 Risograph palette/typography/motion은 deprecated.
> 새 palette/typography/motion 값은 `/bmad-create-ux-design` 신규 라운드에서 lock.
> 본 섹션은 *결정된 가드*만 보존, 구체 토큰 값은 TBD 마커로 둠.
> Risograph 사양은 `archive/ux-design-specification-v1-risograph-2026-05-10.md` 보존.

### Color System (TBD — Oxblood Editorial 방향)

- **Key color**: oxblood / red-adjacent warm. 구체 oklch 값은 design 라운드 산출물.
- **Base**: dark luxury — ink-deep dark + cream/bone secondary 가능 (variant TBD).
- **Brand tokens (구조 골격)**: `key`, `ink`, `paper` (또는 `surface-dark`),
  `cream` (또는 `surface-light`), `accent-1`, `accent-2`, `muted`. 실제 hex/oklch는
  design 라운드에서 lock 후 `tokens.json` (Architecture §4.16)에 기록.
- **Border / shadow**: 두께·offset 값 TBD; dark luxury 톤은 subtle blur / metallic
  accent 허용 (Risograph의 hard-offset only 가드 해제됨).

**L2 Survival State Mapping — packed type 보존**:

| 상태 | label (lock) | icon (TBD) | grass-treatment |
|---|---|---|---|
| `ACTIVE` | "활동 중" | ✓ (또는 동급) | full saturation |
| `YELLOW` | "노란 카드" | ⚠ (또는 동급) | full saturation |
| `RED` | "빨간 카드" (label 유지) | ❤ 또는 동급 (죽음 아이콘 ❌) | full saturation |
| `SPECTATOR` | "관전 중" | ◐ 또는 동급 | dimmed treatment (TBD) |

color 값은 TBD; **alarm/blood 미학 트리거하는 hue/chroma 조합 금지** (PRD §6.1
dignity-color guard). 소비 코드는 `semantic.survival.{state}.color`를 label 없이 참조 ❌
(Architecture §4.15 lint hard gate).

**L3 Emotion Mapping**: 7 design principle 매핑 구조 유지; 시각 토큰 값은 design 라운드.

### Typography System (TBD — Editorial 방향)

- **Primary KR + EN**: Pretendard 유지 (KR/EN 통합 + 9 weight 제공 + Apple SD Gothic
  라이선스 회피). 변경은 design 라운드에서만.
- **Secondary mono**: IBM Plex Mono KR 유지 검토 (Wallet ledger 영수증 톤 — 토스 영감).
  변경은 design 라운드.
- **M3 손글씨**: Gowun Dodum (또는 동급) 유지 검토. 다른 핸드라이팅으로 swap 가능.
- **Type Scale (4px base)**: token names 유지 — `display` / `h1` / `h2` / `h3` /
  `body-lg` / `body` / `caption` / `mono-body` / `handwriting`. 구체 size·line-height는
  design 라운드 mockup 검토 후 확정.
- **Weight 사용 규약**: Regular 400 / Medium 500 / Bold 700 / ExtraBold 800 — Editorial
  hierarchy 강화 시 다른 weight 추가 가능.
- **1인칭 복수 톤 규약** 유지 (PRD FR-8.8.2 lexicon 준수): "우리의" / "함께" / "우리 방".
...
**Layout Principles** (유지):
1. Single column, mobile-first (16px outer padding).
2. Thumb-zone first — 핵심 CTA(daily entry, friend gift) 화면 하단 30%.
3. Card-driven hierarchy.
4. ⚠ "Hard offset shadow only (blur ❌)" 가드 **해제** — dark luxury 톤은 subtle blur /
   metallic accent 허용; 정확한 shadow scale은 design 라운드.
5. No grid system (RN flex column).

**Component spacing** (유지):
...
### Motion Foundation (구조 유지, 값 TBD)

motion token names 유지 (`tap-fast` / `default` / `narrative-soft` / `narrative-medium` /
`mythic-pause` / `ritual-shift`). 구체 duration/easing은 design 라운드 산출물.

**제약 유지**: Reanimated 3 layer 합성 + Skia ❌ (Amelia step-02 lock). RN `Animated`
API + react-native-svg + `Image` opacity transition만 사용.

### Accessibility Considerations (NFR-9.6.*) — 가드 유지

- **NFR-9.6.1** — `semantic.survival` packed type + CI lint hard gate (PRD §14.2 lock).
  소비 코드는 color 필드를 label 없이 참조 불가 (Architecture §4.15).
- **WCAG 2.2 AA contrast** — 새 palette 기준 재검증 필수 (오류 방지를 위해 design 라운드
  마지막에 contrast matrix 산출 + Story 1.5 AC).
- **Dynamic Type** 1.0–1.5x 지원, 1.5x 초과 layout cap.
- **Reduced motion** — M3 5초 시퀀스 → 1초 단축, ritual shift 비활성화 (Story 1.7
  RitualMoment AC).
- **Voiceover / TalkBack** — accessibilityLabel 모든 핵심 CTA 명시.
- **Touch target** ≥ 44×44 (iOS) / 48×48 (Android).
- **a11y audit** step-13 `a11y-architect` 에이전트 환류.

## Design Direction Decision

> **REVISED 2026-05-10**. v1 6 sub-mode 탐색 (D1-D6, D6 cut) 결과는
> `archive/ux-design-specification-v1-risograph-2026-05-10.md` 보존.
> 새 sub-mode mockup은 `/bmad-create-ux-design` 신규 라운드 산출물.
> 본 섹션은 *결정된 골격* 만 명시.

### Design Directions to Explore (in design 라운드)

Dark Luxury × Editorial 융합 위에서 surface별 운용을 위한 sub-mode 후보 5종:

| candidate | 핵심 미학 (방향) | best for |
|---|---|---|
| **D1 Editorial Spread** | 큰 wordmark, 잡지 표지, 강한 type | Final-3 포스터, 월간 ceremony, ASO 공유 |
| **D2 Bento Density** | 카드 모자이크, 정보 풍부 | Wallet 4-track, Pool 표시 |
| **D3 Quiet Dark** | 큰 여백, 한 줄 한 정보, ink-deep | Daily entry, Spectator, Ritual moment |
| **D4 Postcard Mythic** | mythic 톤, paper warmth, handwriting partial | M3 부활 시퀀스, M3.5, Friend Gift |
| **D5 Plate System** | 두꺼운 보더 + metallic accent, administrative 톤 | Rule change, system message |

→ design 라운드에서 5개 모두 mockup 후 surface-mode 매핑 lock.

### Chosen Approach — Surface-Hybrid (구조 유지)

단일 sub-mode 강요는 7 design principle 충돌. surface별 hybrid 매핑 유지.

surface ↔ sub-mode 매핑 표는 design 라운드 산출물 (mockup 검토 후 확정). 후보 매핑:

| Surface | Sub-mode 후보 | 근거 (7 design principle 매핑) |
|---|---|---|
| Today / Daily entry | D3 Quiet Dark | Daily, dignified · ritual sacred |
| Onboarding 5스크린 | D3 + D4 mix | 절제된 안내 + Wallet preview는 mythic |
| Wallet (4-track) | D2 Bento Density | 정보 밀도 · co-presence 신호 |
| Pool 표시 (어디서나) | D2 + animation token | 항상 visible · co-presence |
| Spectator mode | D3 dimmed variant | Loss paused · show don't shame |
| Friend Gift Modal | D4 Postcard Mythic | Invitation one-time · agency · handwriting partial |
| M3 부활 시퀀스 | D4 Postcard ink 톤 | Substitutionary sacrifice 신화 톤 |
| M3.5 받은 자가 주는 자 | D4 Postcard + display | Hero's Return cycle 1회 lifetime |
| Final-3 포스터 | D1 Editorial Spread | Joy shared · "우리의 14일" 1인칭 복수 |
| Room 채팅 | D5 + KakaoTalk 컨벤션 | Administrative · 단톡방 톤 |
| Rule-change 배너 | D5 Plate System | Administrative — sacred ritual ❌ |
| Room invite preview card | D1 Editorial + Kakao SDK 호환 | 외부 노출 가시성 · ASO |
| Welcome window (J0) | D3 Quiet Dark | Leader 외로움 톤, 압박감 zero |

### Design Rationale (구조 유지)

1. **단일 톤 강요는 7 principle 충돌**.
2. **각 surface는 단 하나의 *주된* principle에 봉사**.
3. **surface-mode 매핑 = component-mode 매핑** — 컴포넌트는 자기 surface의 sub-mode 톤으로 build.
4. **"새롭다"가 아니라 "다르게 따뜻하다"** — established UI 컨벤션 위에 sub-mode 변주를
   *조용히* 얹는 전략 (단, dark luxury 톤은 *조용한 위엄*; 따뜻함과 양립 가능).

### Implementation Approach (구조 유지)

1. **L1 token layer**에 `theme.subMode` flag 추가:
   - `'editorial' | 'bento' | 'quiet' | 'postcard' | 'plate'`
   - 각 sub-mode는 token override 집합 정의 (Architecture §4.16 codegen schema).
2. **L3 component**는 prop `subMode` 받아 self-render. 디폴트는 'quiet'.
3. **Page-level wrapper (Screen)**는 자기 화면 sub-mode를 암시적 주입.
4. **Final-3 SVG renderer (BE)**는 D1 Editorial sub-mode 토큰을 **codegen** 으로 소비
   (Architecture §4.16) — FE/BE drift 구조적 불가능.
5. **a11y 검증** — sub-mode마다 contrast / dynamic type / reduced motion 별도 verify
   (Story 1.5 AC).

### Visual Showcase

`ux-design-directions.html` v1 (Risograph)은 archive로 보존
(`archive/ux-design-directions-v1-risograph-2026-05-10.html` — design 라운드 시작 시 archive로 이동).
v2 (Oxblood Editorial) viewer는 design 라운드 산출물 — 5 sub-mode mockup 포함
...
> **REVISED 2026-05-10** via Sprint Change Proposal. 컴포넌트 *이름·prop·anatomy* 는 새
> 시스템 lock 후 design 라운드 산출물로 재작성. 본 섹션은 (a) U1-U9 disposition lock,
> (b) story ownership lock, (c) implementation roadmap 골격만 보존.

### Component Inventory

Atom/molecule 10 + Pattern 8 = **18 컴포넌트** (working names — design 라운드에서 재명명 가능).
본 SCP에서 lock하는 것은 **disposition (accept / merge / cut) 과 story ownership** 만.

**Atoms / Molecules (Layer 3)**

| 컴포넌트 (working name) | Purpose | Disposition | Story ownership |
|---|---|---|---|
| `<RisoButton>` (rename TBD) 🔁 | Primary CTA / key-emphasis / ghost | **ACCEPTED** | Story 1.5 (token) |
| `<RisoCard>` (rename TBD) 🔁 | Generic card wrapper, prop `subMode` | **ACCEPTED** | Story 1.5 |
| `<RisoSheet>` (rename TBD) ✨ | Bottom modal sheet base | **ACCEPTED** | Story 1.5 → used by 3.2 |
| `<PoolMeter>` ✨ | 그룹 점수 5단계 SVG/PNG swap | **ACCEPTED** | Story 4.1 (AC 확장) |
| `<NoiseOverlay>` ✨ | grain layer (PNG noise + tintColor) | **DEFERRED to v1.5** — Risograph 전용 primitive, oxblood editorial 방향에선 필수 ❌; 디자인 라운드에서 keep/cut 재결정 | (none) |
| `<HardShadow>` 🔁 | Hard-offset shadow utility | **CONDITIONAL** — dark luxury에서 subtle blur 허용으로 락 해제됨; design 라운드에서 필요성 재평가 | (none — 사라질 가능성) |
| `<GrassGrid>` 🔁 | 잔디 grid + spectator dimmed variant | **ACCEPTED** | Story 2.1 (AC 확장) |
| `<SystemMessage>` 🔁 | Chat 시스템 메시지 + rule-change tone | **ACCEPTED** | Story 5.4 (AC 확장) |
| `<RitualMoment>` ✨ (U4) | 06:00 KST 5초 sacred ritual wrapper | **ACCEPTED** | Story 1.7 (NEW) |
| `<KudosButton>` ✨ (U3) | "응원만 보내기" (0점) | **ACCEPTED** | Story 3.5 (NEW) + Story 3.2 AC |

**Patterns (Layer 4)**

| 컴포넌트 (working name) | Disposition | Story ownership |
|---|---|---|
| `<SurvivalBanner>` | **ACCEPTED** | Story 1.3 (AC 확장) |
| `<Wallet>` 4-track | **ACCEPTED** | Story 3.4 (AC 확장 — 4-section 명시) |
| `<FriendGiftModal>` 3-CTA | **ACCEPTED** | Story 3.2 (AC 확장 — 3 CTA + 안심 + Kudos) |
| `<RevivalSequence>` 5-phase | **ACCEPTED** (M3 mythic 3-5s) | Story 3.2 (AC 확장 — 5-phase animation 명시) |
| `<ReceivedGiftToast>` | **ACCEPTED** | Story 3.2 |
| `<FinalThreeCard>` D1 Editorial | **ACCEPTED** | Story 7.3 (AC 확장 — D1 sub-mode 명시) |
| `<RoomInviteSheet>` Kakao | **ACCEPTED** | Story 6.2 |
| `<WelcomeWindow>` ✨ (U1) | **ACCEPTED** | Story 1.6 (NEW) |

### U1–U9 Disposition Summary (PRD-level)

| # | Element | Disposition | Authoritative ref |
|---|---|---|---|
| **U1** | J0 + `<WelcomeWindow>` | ACCEPT (a) — 새 PRD §4.3 J0 + Story 1.6 | PRD §4.3 J0, Story 1.6 |
| **U2** | M3.5 received-becomes-giver lifetime-1 | ACCEPT (a) — Story 3.2 AC 확장 (lifetime-1 marker on first FRIEND_GIFT receive→send) | Story 3.2 AC |
| **U3** | `<KudosButton>` 0점 응원 | ACCEPT (a) — FR-8.3.9 + Story 3.5 + Story 3.2 AC | PRD FR-8.3.9, Story 3.5 |
| **U4** | `<RitualMoment>` 06:00 KST | ACCEPT (a) — Story 1.7 (root authority PRD §6.4 principle 5) | Story 1.7 |
| **U5** | 48h post-revival recovery window | **DEFER to v1.5** — streak_freezes 의미 변경 필요; v1 budget 압박 |
| **U6** | Donor-protection signal | **DEFER to v1.5** — telemetry 후 Day-30 재평가 |
| **U7** | Auto digital sabbatical | **DEFER (conflicts with FR-8.3.1 lifetime-1 free ticket)** — phase-2 재평가 |
| **U8** | 7-day echo footnote on receiver daily-entry | ACCEPT (a) — Story 3.2 AC 확장 (footer footnote rendering + 7-day decay) | Story 3.2 AC |
| **U9** | Spectator gentle prompt (negative-content) | **DEFER to v1.5** — content analyzer infra 비용 |

### Custom Component Specifications

> **DEPRECATED 2026-05-10**. Risograph-specific anatomy 명세 (`<RisoSheet>` / `<PoolMeter>`
> / `<NoiseOverlay>` / `<RitualMoment>` / `<KudosButton>` / `<SurvivalBanner>` /
> `<FriendGiftModal>` / `<RevivalSequence>` / `<FinalThreeCard>` / `<WelcomeWindow>`)
> 는 archive (`archive/ux-design-specification-v1-risograph-2026-05-10.md`)로 보존.
> design 라운드 산출물에서 새 anatomy로 재작성.
> Disposition + Story ownership은 위 §Component Inventory 표에서 lock 됨.
>
> 신규 anatomy 명세 시 유지 가드:
> - **`<FriendGiftModal>` 3-CTA 동등 비중** (회생권 / Kudos / 닫기) — Story 3.2 AC.
> - **M3 5-phase 시퀀스** + reduced-motion 1s fade fallback — Story 3.2 AC.
> - **`<KudosButton>` 0점 + 메시지 (선택, max 60chars)** — Story 3.5 + FR-8.3.9.
> - **`<RitualMoment>` 06:00–06:05 KST window + 변형 텍스트 + reduced-motion 1s** — Story 1.7.
> - **`<WelcomeWindow>` solo→growing→full state machine** + 2 CTA 동등 + 진행 막대 ❌ — Story 1.6.
> - **`<FinalThreeCard>` surviving-member-only visibility** + dignity 보존 — Story 7.3.
> - **`<SurvivalBanner>` packed `semantic.survival` token** + label 항상 visible — Story 1.5 / 1.3.

### Implementation Roadmap (W1–W7 매핑) — UPDATED 2026-05-10

W4까지 atom/molecule 모두 land해야 W5–W7에서 pattern을 token+atom *조합만*으로 land 가능.

```
W1 (Epic 1 Token + base + design 라운드 lock)
  📐 /bmad-create-ux-design 라운드 lock — palette/typography/motion 토큰 값 확정
  📐 docs/design-system.md 재작성
  📐 ux-design-directions.html v2 재제작 (5 sub-mode mockup)
  L1 ✦ token packed type (semantic.survival) + FE→BE codegen pipeline (Architecture §4.16)
  L2 ✦ primitive (TBD — NoiseOverlay 대체 또는 keep)
  L3 ✦ Button / Card 확장 (working name TBD)
  📦 Story 1.5 (Design System Foundation v2) lock

W2 (Epic 1 + 시작 Epic 2)
  L3 ✦ Sheet / GrassGrid (dimmed variant) / SurvivalBanner / SystemMessage
  📦 Story 1.6 (WelcomeWindow J0) build

W3 (Epic 3 + Epic 5)
  L3 ✦ FriendGiftModal + KudosButton + ReceivedGiftToast + RuleChangePreview
  📦 Story 3.5 (Kudos endpoint BE + chat_messages kind) build
  📦 Story 3.2 AC 확장 (3 CTA + Kudos + 안심 + lifetime-1 M3.5 + 7-day footnote)

W4 (Epic 3 finish + Epic 4)
  L3 ✦ PoolMeter
  L4 ✦ Wallet 4-track (Bento) + RevivalSequence 5-phase + M3.5 모먼트

W5 (Epic 6 Kakao SDK)
  L3 ✦ Kakao Share SDK wrapper (native module)
  L4 ✦ RoomInviteSheet 확장 + Preview card BE renderer (codegen 사용)

W6 (Epic 7 Final-3 + RitualMoment)
  L4 ✦ FinalThreeCard D1 Editorial (BE 토큰 codegen 사용)
  📦 Story 1.7 (RitualMoment) build

W7 (Epic 8 Brand Voice + Onboarding)
  L4 ✦ Onboarding 5스크린 (S1-S5)
  L4 ✦ tools/brand-voice-lint.ts 확장 (semantic.survival lint hard gate)
  L4 ✦ Reduced-motion 폴백 audit + a11y-architect 환류
```

### W1 Spec Lock Items (시작 전 합의 필수) — UPDATED 2026-05-10

5 lock items (Sprint Change Proposal 2026-05-10 기준):

1. **Design 라운드 lock** — `/bmad-create-ux-design` 산출물 (palette + typography + 5 sub-mode mockup + tokens.json) 사용자 sign-off.
2. **FE→BE token codegen pipeline** — Architecture §4.16 합의 + Gradle `generateTokens` task 구현 완료.
3. **WS event schema `gift.revive.*` + `kudos.sent`** — BE/FE 합의 (Kudos 이벤트 추가됨).
4. **Analytics SDK 선정** — onboarding.screen.dwell_ms / friend-gift conversion / spectator→revival / kudos send rate 측정 가능해야 함. *현재 미선정* — W1 첫주 결정 필수.
5. **NFR-9.6.1 lint spec** — `tools/brand-voice-lint.ts` 확장 spec 합의 (Architecture §4.15: AVOID lexicon warn + `survival.*.color` packed-type hard gate + design-token literal guard).
