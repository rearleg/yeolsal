...
- [x] **FE-9** Lint clean on every touched file (verified `npx eslint <8 files>` returns 0 problems). Repo-wide `npm run lint` has 4 pre-existing errors + 2 warnings in files this story did not touch (`react-hooks/exhaustive-deps` plugin missing in `app/rooms/[id]/chat.tsx` + `lib/realtime/client.ts`; `SurvivalChip*` `require()` imports; `InviteCodeSheet.test.tsx` unused directive); repo-wide `npm run typecheck` has 2 pre-existing errors in `FriendsTodayPager.tsx` (missing `react-native-pager-view`). All pre-existing — verified via `git stash` cycle. Repo-wide `npm test` green: 53 suites / 335 / 335. See deferred-work entry below.

### Scripts / verification / sprint-status

- [~] **VERIFY-1** `bash scripts/verify.sh` does NOT pass end-to-end on the current repo state: the FE lint stage fails on 4 pre-existing errors unrelated to Story 4.1 (verified by `git stash` of all Story 4.1 changes → identical 4 lint errors + 2 warnings remain). BE `./gradlew test` is green (77 classes / 0 failures), FE `npm test` is green (53 suites / 335/335). The FE lint baseline is the blocker — tracked in `deferred-work.md` for a separate `chore(infra)` PR.
- [~] **VERIFY-2** Manual smoke (dev environment) — DEFERRED. The unit + slice + integration test layer enforces the live-fill semantics (`RoomPointPoolServiceTest.applyDelta_publishesPointPoolChangeEventWithComputedNewTotal` on BE + the `useRoomPoints merges STOMP frame using authoritative newTotal` case on FE), but the end-to-end visual confirmation requires Expo + Spring Boot + Postgres + a second device. Recommended before public-build cut.
- [x] **VERIFY-3** `gh pr view 82 --json baseRefName` returned `"main"` at PR-open time. ✓
...
- [x] [Review][Patch] Reject older unique STOMP frames so delayed delivery cannot regress the cached pool total [FE/src/lib/query/hooks/roomPoints.ts:71] — applied: compare `frame.occurredAt < prev.lastEventAt` inside `setQueryData` updater; verified by new `roomPoints.test.tsx > Patch 1` case (newer frame at 05:00Z stays even after older 04:00Z frame).
- [x] [Review][Patch] Reconcile or cancel an in-flight REST snapshot when a newer STOMP frame is merged so the delayed response cannot overwrite live state [FE/src/lib/query/hooks/roomPoints.ts:74] — applied: `qc.cancelQueries({ queryKey: qk.roomPoints(roomId) })` called before `setQueryData` inside the STOMP handler; verified by new `roomPoints.test.tsx > Patch 2` case (`cancelQueries` spy invoked with the right key).
- [x] [Review][Patch] Refresh roomPoints after realtime reconnect or provide a disconnected fallback so externally generated increments missed during a WS gap are recovered [FE/src/lib/query/hooks/roomPoints.ts:43] — applied: `useRealtimeStatus` consumed; `useRef<RealtimeStatus>` tracks the previous status across rerenders; `useEffect` invalidates `qk.roomPoints(roomId)` on `disconnected → connected` transition. Verified by new `roomPoints.test.tsx > Patch 3` case (`invalidateQueries` spy invoked after `mockUseRealtimeStatus` flip).
- [x] [Review][Patch] Update or invalidate qk.roomPoints(roomId) after successful self-revival and friend-gift mutations so the caller recovers when realtime delivery is unavailable [FE/src/lib/query/hooks/revival.ts:28] — applied to both `useSelfRevival.onSuccess` (`FE/src/lib/query/hooks/revival.ts`) and `useSendFriendGift.onSuccess` (`FE/src/lib/query/hooks/friendGift.ts`). Verified by extended assertions in `revival.test.tsx` and `friendGift.test.tsx`.
- [x] [Review][Patch] Use survival.roomPointPool as the Wallet fallback while the dedicated points query is loading or failed instead of displaying a false zero [FE/src/components/wallet/WalletScreen.tsx:150] — applied: `pool` derives from `(roomPoints.isLoading || roomPoints.isError) ? survival.roomPointPool : roomPoints.total`. Verified by two new `WalletScreen.test.tsx > Patch 5` cases (loading-pending → renders `42`; rejected REST → renders `7`).
- [x] [Review][Patch] Correct verification checkboxes and completion claims: VERIFY-1 did not pass end-to-end, VERIFY-2 and VERIFY-3 were deferred, and FE-9 is not green repository-wide [_bmad-output/implementation-artifacts/4-1-room-point-pool-counter-cache.md:343] — applied: VERIFY-1 marked `[~]` (FE lint baseline blocks repo-wide pass; root cause traced to pre-existing files), VERIFY-2 marked `[~]` (deferred — requires live dev env), VERIFY-3 marked `[x]` (PR #82 opened with base=main; verified via `gh pr view`), FE-9 reworded to scope to touched-file lint clean + repo-wide test green; pre-existing lint/typecheck baseline failures listed separately and tracked in `deferred-work.md`.
- [x] [Review][Defer] Repair the pre-existing FE repository verification failures: missing react-native-pager-view typing/dependency and unrelated lint failures [_bmad-output/implementation-artifacts/4-1-room-point-pool-counter-cache.md:584] — deferred, pre-existing
